...
@Composable
fun ChatScreen(onOpenDocsClick: (() -> Unit),onOcrClick: (() -> Unit),onOcrScreenClick: (() -> Unit)) {
    DocQATheme {
        Scaffold(
            modifier = Modifier.fillMaxSize(),
            topBar = {
                TopAppBar(
                    title = { Text(text = "new Chat", style = MaterialTheme.typography.headlineSmall) },
                    actions = {
...
//                            )
//                        }
                        IconButton(onClick = onOcrScreenClick) {
                            Icon(
                                imageVector = Icons.Default.Search, // 使用搜索图标
                                contentDescription = "Open OCR"
                            )
                        }
                    }
                )
...
private fun QueryInput(chatViewModel: ChatViewModel) {
    ...
    Row(verticalAlignment = Alignment.CenterVertically) {
        TextField(
            ...
            placeholder = { Text(text = "new Ask documents...") }
        )
